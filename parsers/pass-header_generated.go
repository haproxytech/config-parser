...
func (p *PassHeader) Delete(index int) error {
	...
	p.data[len(p.data)-1] = types.PassHeader{}
...
func (p *PassHeader) Insert(data common.ParserData, index int) error {
	...
	case *types.PassHeader:
		if index > -1 {
			...
			p.data = append(p.data, types.PassHeader{})
...
func (p *PassHeader) PreParse(line string, parts []string, preComments []string, comment string) (changeState string, err error) {
