...
func (p *SetVarFmt) Delete(index int) error {
	...
	p.data[len(p.data)-1] = types.SetVarFmt{}
...
func (p *SetVarFmt) Insert(data common.ParserData, index int) error {
	...
	case *types.SetVarFmt:
		if index > -1 {
			...
			p.data = append(p.data, types.SetVarFmt{})
...
func (p *SetVarFmt) PreParse(line string, parts []string, preComments []string, comment string) (changeState string, err error) {
...
func (p *SetVarFmt) Parse(line string, parts []string, comment string) (changeState string, err error) {
	if parts[0] == "set-var-fmt" {
		data, err := p.parse(line, parts, comment)
		if err != nil {
			if _, ok := err.(*errors.ParseError); ok {
				return "", err
			}
			return "", &errors.ParseError{Parser: "SetVarFmt", Line: line}
		}
