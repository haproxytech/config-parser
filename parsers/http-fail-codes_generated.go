...
func (p *HTTPFailCodes) Delete(index int) error {
	...
	p.data[len(p.data)-1] = types.HTTPFailCodes{}
...
func (p *HTTPFailCodes) Insert(data common.ParserData, index int) error {
	...
	case *types.HTTPFailCodes:
		if index > -1 {
			...
			p.data = append(p.data, types.HTTPFailCodes{})
...
func (p *HTTPFailCodes) Parse(line string, parts []string, comment string) (string, error) {
	if parts[0] == "http-fail-codes" {
		...
		if err != nil {
			...
			return "", &errors.ParseError{Parser: "HTTPFailCodes", Line: line}
		}
