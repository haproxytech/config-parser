...
func (p *EmailAlert) Delete(index int) error {
	...
	p.data[len(p.data)-1] = types.EmailAlert{}
...
func (p *EmailAlert) Insert(data common.ParserData, index int) error {
	...
	case *types.EmailAlert:
		if index > -1 {
			...
			p.data = append(p.data, types.EmailAlert{})
...
func (p *EmailAlert) PreParse(line string, parts []string, preComments []string, comment string) (changeState string, err error) {
...
func (p *EmailAlert) Parse(line string, parts []string, comment string) (changeState string, err error) {
	if parts[0] == "email-alert" {
		data, err := p.parse(line, parts, comment)
		if err != nil {
			return "", &errors.ParseError{Parser: "EmailAlert", Line: line}
		}
