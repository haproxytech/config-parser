...
func (p *SetVar) Delete(index int) error {
	...
	p.data[len(p.data)-1] = types.SetVar{}
...
func (p *SetVar) Insert(data common.ParserData, index int) error {
	...
	case *types.SetVar:
		if index > -1 {
			...
			p.data = append(p.data, types.SetVar{})
...
func (p *SetVar) PreParse(line string, parts []string, preComments []string, comment string) (changeState string, err error) {
...
func (p *SetVar) Parse(line string, parts []string, comment string) (changeState string, err error) {
	if parts[0] == "set-var" {
		data, err := p.parse(line, parts, comment)
		if err != nil {
			return "", &errors.ParseError{Parser: "SetVar", Line: line}
		}
